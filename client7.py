...
# 사용자 정의 이름 저장을 위한 세션 상태 추가
# 이제 이 값은 폼 제출 후에만 업데이트될 것입니다.
...
with st.form("conversation_params_form"):
    # 이름을 직접 텍스트로 입력받도록 변경
    # on_change 콜백 제거
    current_person_name_input = st.text_input(
        "대화할 사람 이름",
        value=st.session_state.person_name_input_value, # 세션 상태에 저장된 값 사용
        key="person_name_input_widget_key", # 위젯의 키 (폼 내에서는 고유해야 함)
    ...
    if age_group_val == "not_target":
        ...
        # 'key'를 사용하여 selectbox의 값을 고유하게 관리
...
# --- 대화 생성 로직 ---
if submitted:
    # 폼이 제출되면 text_input의 현재 값을 세션 상태에 저장하고 person_name_final에 할당
    ...
    if not person_name_final.strip(): # 이름이 비어있는지 확인
        st.error("대화할 사람 이름을 입력해주세요.")
        st.session_state.is_generating = False # 오류 발생 시 생성 상태 해제
...
if st.session_state.full_report_data:
    # 폼 외부에서 person_name_final을 사용하기 위해 세션 상태에서 가져옴
