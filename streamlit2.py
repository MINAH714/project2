...
for group, items in raw.items():
    for idx, item in enumerate(items):
        # 실제 감정은 item['emotion']에 있음!
        rows.append({
            "timestamp": now - pd.Timedelta(minutes=len(rows)),  # 최근순으로 시간 부여
            "group": group,
            "emotion": item.get("emotion", ""),
            "strength": np.random.uniform(0.3, 1.0),  # 감정 세기 랜덤
...
scatter = alt.Chart(df).mark_circle(size=80).encode(
    ...
    tooltip=['timestamp:T', 'emotion:N', 'strength:Q', 'content:N', 'group:N']
).properties(width=900, height=350).interactive()
st.altair_chart(scatter, use_container_width=True)

# 일주일 감정 변화 선그래프
st.subheader("🟦 일주일 감정 변화 추이")
one_week_ago = df['timestamp'].max().date() - pd.Timedelta(days=7)
week_df = df[df['date'] > one_week_ago].copy()
