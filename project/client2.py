...
num_dialogues_per_step = st.number_input("회차당 대화 갯수", min_value=1, value=4) #
...
if st.button("AI 대화문 생성"):
    if situation is None:
        ...
        # 이 부분은 FastAPI 서버가 배포된 실제 주소로 변경해야 합니다.
        fastapi_url = "http://localhost:8000/generate_dialogues" # FastAPI 서버 주소

        try:
            response = requests.post(fastapi_url, json=request_payload, timeout=300) # 타임아웃 5분
            response.raise_for_status() # HTTP 오류 발생 시 예외 발생

            result_data = response.json()

            if result_data.get("status") == "success":
                generated_dialogues_data = result_data.get("data", [])
                st.subheader("생성된 대화 리포트")

                for entry in generated_dialogues_data:
                    st.write(f"**날짜: {entry['date']}**")
                    st.write(f"**대화 내용:** {entry['dialogue']}")
                    st.json(entry) # 전체 JSON 내용도 표시

                # 전체 리포트를 하나의 JSON 파일로 다운로드
                json_str = json.dumps(generated_dialogues_data, ensure_ascii=False, indent=2)
                st.download_button(
