...
with st.form("conversation_params_form"):
    ...
    # Changed from st.selectbox to st.text_input for situation
    situation = st.text_input(
        "상황",
        placeholder="대화 상황을 입력하세요 (예: 학교 생활, 직장 생활, 건강 관리)...",
        key="situation_text_input"
    )
    
    # Removed the logic related to fetching and validating situation options
    # as situation is now a direct text input.

    start_date_input = st.date_input("기준 시작 날짜 (YYYY-MM-DD)", value=date(2025, 6, 1))
    start_date_str = start_date_input.isoformat()

    # Fixed values as per previous request
    step_days = 1
    num_conversations = 1
...
if submitted:
    ...
    if not person_name_final.strip():
        ...
        st.session_state.is_generating = False
    elif not situation.strip(): # Validate text input for situation
        st.error("상황을 입력해주세요.")
        st.session_state.is_generating = False
    else:
